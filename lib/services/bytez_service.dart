...
class BytezService {
  ...
  BytezService() {
    _dio = Dio(BaseOptions(
      baseUrl: AIConstants.bytezBaseUrl,
  ...
  Future<String> _getApiKey() async {
    ...
    final apiKey = dotenv.env['BYTEZ_API_KEY'];
    if (apiKey == null || apiKey.isEmpty) {
      throw ApiKeyException('Bytez API Key not found in .env');
    }
    return apiKey;
  }
  ...
  Future<Map<String, dynamic>> analyze(String systemPrompt, String problemText, {String category = 'general'}) async {
    try {
      ...
      final response = await _dio.post(
        ...
        options: Options(headers: {'Authorization': apiKey}), // Bytez uses bare key usually or Bearer? Earlier test used bare key in header.
        data: {
          'model': AIConstants.bytezModel,
          'messages': [
      ...
      return _parseJsonResponse(response, 'bytez', AIConstants.bytezModel);
    } catch (e) {
      _handleDioError(e, 'Bytez');
    }
  }
